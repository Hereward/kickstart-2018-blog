...
* `allowMultiSession` - allow multiiple client sessions for the same user. **Note:** using this setting in conjunction with `enhancedAuth: {active: true}` presents a security risk as the auth token is not client specific
